...
object RimeConfigHelper {
    ...
    fun initializeRimeData(context: Context): Pair<String, String> {
        ...
        // 检查是否需要重新部署（检查 prism.bin 是否存在）
        val buildDir = File(userDataDir, "build")
        val prismFile = File(buildDir, "wubi86.prism.bin")
        val tableFile = File(buildDir, "wubi86.table.bin")
        
        val needDeploy = !prismFile.exists() || !tableFile.exists()
        
        // 如果需要部署，清理 build 目录
        if (needDeploy && buildDir.exists()) {
            Log.d(TAG, "Prism or table file missing, cleaning build directory to force redeploy")
